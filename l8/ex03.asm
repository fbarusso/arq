...
main:	la $a0, linhasMatriz	# carrega o endereço da string
	...
	jal getValor		# getValor(matriz, numeroLinhas, numeroColunas)
	...
	la $a0, resultadoVerdadeiro# carrega o endereço da string
	li $v0, 4		# código de impressão de string
	syscall			# imprime a string
...
gV:	jal indice		# calcula o endereço de matriz[i][j]

	lw $t7, ($v0)		# t7 = valor de matriz[i][j]
	
	add $s1, $s1, $t7	# valor = valor + valor em matriz[i][j]
	
	addi, $t1, $t1, 1	# j++
	
	blt $t1, $a2, gV	# if (j < numeroColunas) goto pL
...
false:	la $a0, resultadoFalso	# carrega o endereço da string
	li $v0, 4		# código de impressão de string
	syscall			# imprime a string
